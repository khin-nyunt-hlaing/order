...
<div class="titlebox" style="height=30px;">
    <p1><?= $mode === 'edit' ? 'お知らせ編集' : '社内共有情報入力画面' ?></p1>
    <?= $this->Flash->render() ?>
</div>
...
<div class="TAbox">
            ...
            <?php if ($mode === 'edit'): ?>
                <div>
                    ...
                    <?= $this->Form->control('user_group_id', [
                        ...
                        'empty' => '選択してください',
                        'options' => $groupList,
                        'value' => $selectedGroupId // 初期値（必要であれば）
                    ]) ?>
                </div>
    ...
    <div class="TAminibox" style='align-items: flex-start; height:100px; gap:0 !important; margin-bottom:10px;'>
        <p style=' margin-right:3rem;'>添付ファイル</p>
             <!-- 左カラム（1〜3） -->
                <div style="display: flex; flex-direction: column; gap: 0.5rem; padding-right:10px;">
                    <?php for ($i = 1; $i <= 3; $i++): ?>
        ...
        <div class="TAnnounceEd5">
            <?= $this->Form->button($mode ? '更新' : '登録', ['id' => 'ANNOUNCEreg', 'class' => 'akabtn-like']) ?>
        ...
        if (serviceCode || groupCode) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#RecipientView').html(response);
                    $('#RecipientView').css('background', '#e0f7fa');  // ← 背景色をつけて、動作確認
                },
                error: function(xhr) {
                    console.error("初回Ajax失敗", xhr.status);
                }
            });
        }

        // ▼ セレクト変更時にAjax呼び出し
        $('#sService, #groupList').on('change', function() {
            var serviceCode = $('#sService').val();
            var groupCode = $('#groupList').val();
            console.log('[JS] groupCode送信値:', groupCode);
...
<script>
    ...
    // ファイル名表示（1つ目のみ）
    const input = document.getElementById('attachment1');
    const label = document.getElementById('file-name-label-1');
    if (input && label) {
